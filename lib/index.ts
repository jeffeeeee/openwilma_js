    ...
    async login(account: WilmaAccountConfiguration, validateServer: boolean, versionCheck: boolean) : Promise<void | WilmaAccountInstance> {
        try {
            ...
            if(validateServer != false){
                const servers: WilmaServer[] = await listServers()
                const result = servers.find(({url, formerUrl}) => url === account.server || formerUrl == account.server)
                if(result !== undefined) {
                        ...
                        if(req.headers["set-cookie"]) {
                            ...
                            for (const cookie of req.headers["set-cookie"]) {
                                if(cookie.includes("Wilma2SID")) {
                                    // Wilma COOKIE
                                    const sessionId = /^(.*)Wilma2SID=([^;]+)(.*)$/.exec(cookie)
                                    if(sessionId !== null) {
                            ...
                            if(creds.status == 200){
                                ...
                                try {
                                    const secret: string = creds.data.split("name=\"secret\" value=\"")[1].split("\"")[0]
                                    const formkey: string = creds.data.split("name=\"formkey\" value=\"")[1].split("\"")[0]
                                    if(secret && formkey){
                                        // We got all credentials
                                        // Build session
                                        const session: WilmaSession = {
                                            id: sessionValue,
                                            secret: secret,
                                            formkey: formkey,
